#!/usr/bin/env node
/**
 * 抓「詞條(被動)」與「主動技」目錄,給指令台的自訂帕魯選單用。含 zh(繁中)/zh-CN(簡中)/ja(日文)名稱。
 *
...
async function main() {
  ...
  const passives = passivesBase.map((p) => {
    ...
    const zhCN = idx !== undefined ? cnPassiveList[idx]?.name : existingPassives.get(p.id)?.["zh-CN"];
    return {
      id: p.id,
      name: p.name,
      ...(zh ? { zh } : {}),
  ...
  for (const [id, name] of namesEn) {
    ...
    const zhCN = namesZhCN.get(id) ?? existingSkills.get(id)?.["zh-CN"];
  ...
  const passiveZhCNCount = passives.filter((p) => p["zh-CN"]).length;
  const skillZhCount = skills.filter((s) => s.zh).length;
  const skillZhCNCount = skills.filter((s) => s["zh-CN"]).length;
